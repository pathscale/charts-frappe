...
const AXIS_DATASET_CHART_TYPES = ['line', 'bar', 'candle'];
...
const CANDLE_CHART_SPACE_RATIO = 0.1;
const MIN_CANDLE_PERCENT_HEIGHT = 0.00;
...
function getCandleAttr(candleInfo, zeroLine) {
...
function animateCandle(candle, x, candleInfo, width, offset=0, meta={}) {
...
function dataPrep(data, type, xUnit =  1) {
...
function zeroDataPrep(realData, type, xUnit = 1) {
	...
	calcDatasetPoints() {
		...
		s.datasets = this.data.datasets.map((d, i) => {
			...
			return {
				...
				values: values,
				yPositions: d.chartType === 'candle' ? scaleCandles(values) : scaleAll(values),
